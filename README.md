...
Complexity: O(nlog⁡n)O(nlogn)
 (Best, Average, and Worst case).
Purpose: Efficiently sorts large datasets by recursively splitting and merging arrays.
2. Binary Search (Searching)
Approach: Decrease and Conquer.
Complexity: O(log⁡n)O(logn).
Purpose: Extremely fast searching in a sorted array by halving the search space at each step.
